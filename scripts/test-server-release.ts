...
async function main() {
  ...
  try {
    ...
    assert.equal(imageRequest({ job, settings } as never).model, 'qwen-image-21');
    ...
    const media = { worker_availability: 'configured; polled on work', models: [{ id: 'qwen-image-21', operations: ['image.generate'] }, { id: 'wan2.2-5b-fp8', operations: ['video.image_to_video'] }, { id: 'ace-step-1.5-turbo', operations: ['music.generate'] }] };
